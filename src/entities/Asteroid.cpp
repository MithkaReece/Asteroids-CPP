...
namespace Entity
{
  ...
  void createAsteroid(Scene::Scene &scene, sf::RenderWindow &window, int level, sf::Vector2f position, sf::Vector2f velocity)
  {
    ...
    scene.emplace<Component::Asteroid>(entity, level);
    scene.emplace<Component::Transform>(entity, position, scaleVector, 0.0f);
    scene.emplace<Component::Velocity>(entity, velocity);
    scene.emplace<Component::WrapperBoundary>(entity, BOUNDARY);
  ...
  void createAsteroid(Scene::Scene &scene, sf::RenderWindow &window)
  {
    ...
    createAsteroid(scene, window, level, position, velocity);
  }
}